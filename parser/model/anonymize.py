...
class Anonymizer:
    ...
    def anonymize(cls, nlp: spacy.language.Language, documents: List, output_folder: os.PathLike) -> None:
        ...
        for i, doc in tqdm(enumerate(nlp.pipe(md_docs, disable=["tagger", "parser", "attribute_ruler", "lemmatizer"])), desc=f"Anonymizing documents", total=len(md_docs)):
            # Anonymize named entities referring to people, products and organizations in the documents by replacing them with their entity type
            anonymized_doc = "".join([f"[{t.ent_type_}]"+t.whitespace_ if t.ent_type_ in ["ORG", "PERSON", "PRODUCT"] else t.text_with_ws for t in doc])
